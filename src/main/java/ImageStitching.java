...
//        }
        ...
        try {
            ...
            return new PositionAndWeight(new MyPosition(Integer.MAX_VALUE, Integer.MAX_VALUE), 1);
        }
...
//        System.out.println("剔除误差后均值： " + offset[0] + "  " + offset[1]);
        PositionAndWeight positionAndWeight = new PositionAndWeight(CalculateOffset(imgA.size(), overlap, (int) offset[0], (int) offset[1], lr), (int) 100 * goodMatches.size() / knnMatches.size());
        imgB.release();
        imgACrop.release();
        imgA.release();
        imgBCrop.release();
...
//        String pathA = "src/photo/up.tif";
//        String pathB = "src/photo/down.tif";
//        Match(pathA, pathB, 0.25f, true);
//        System.exit(0);
...
//        System.out.println(result.size());
        ...
        System.out.println("输入xNums(列数)、yNums(行数)、mode(拍摄路径)\n0:snake by row\n1:row by row down\n2:snake by column\n3:row by row up\n4:column by column right");
        int xNums = sc.nextInt();
        int yNums = sc.nextInt();
        int mode = sc.nextInt();
        long start = System.currentTimeMillis();
//        int xNums = 29, yNums = 21;  //cycle1
//        int xNums = 21, yNums = 25;     //2023409
//        int xNums = 10, yNums = 10;     //20230330_1
        float lrratio=0.3f, upratio=0.3f;
//        String path = "H:\\dataset\\Tak\\Shading-correction-master\\Slide images\\Original\\36-01";
//        String path = "E:\\java\\NewStitch\\src\\photo\\ur";
//        String path = "H:\\dataset\\colon\\cycle1";

//        String path = "E:\\python\\stitch\\ImageStitch\\demoImages\\20230409";
        String path = "E:\\java\\NewStitch\\src\\photo\\20230330_1";
        OffsetsAndWeights offsetsAndWeights = CalculateAllOffset(path, xNums, yNums, lrratio, upratio, mode);
//        int xNums = 2, yNums = 1;
//        OffsetsAndWeights offsetsAndWeights = CalculateAllOffset("src/photo/flour", xNums, yNums, (float)0.2);
        ...
        Imgcodecs.imwrite("./result_" + path.substring(path.length()-4) + ".png", result);
//        HighGui.imshow("result", result);
//        HighGui.waitKey();
//        String[][] files = offsetsAndWeights.fileNames;
//        for(int i = 0; i < files.length; i++) {
//            for(int j =0; j <files[0].length; j++) {
//                System.out.print(brightness.get(files[i][j]) + "\t\t");
//            }
//            System.out.print("\n");
//        }
        System.out.println("拼接用时： "+ (System.currentTimeMillis() - start) + "毫秒");
//        try {
//            // 创建文件输出流
...
//            // 序列化HashMap
//            objectOutputStream.writeObject(brightness);
//            // 关闭流
//            objectOutputStream.close();
//            fileOutputStream.close();
//            System.out.println("HashMap已序列化");
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
